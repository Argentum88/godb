...
func NewSlottedPage(data []byte) (*slottedPage, error) {
	if len(data) <= (headerSize + slotSize) {
		return nil, fmt.Errorf("data size %d is too small for slotted page", len(data))
	}
...
func (sp *slottedPage) insertTuple(slotID uint16, tuple []byte) {
	...
	// Вставляем слот
	slot := newSlot(freeSpacePointer-uint16(len(tuple)), len(tuple), slotUsed)
	copy(sp.data[newSlotPointer:newSlotPointer+slotSize], slot)
...
func (sp *slottedPage) compact() {
	...
	for _, usedTuple := range usedTuples {
		...
		slot := newSlot(freeSpacePointer-uint16(len(usedTuple.tuple)), len(usedTuple.tuple), usedTuple.flags)
		pointerToSlot := headerSize + slotSize*usedTuple.slotID
		copy(sp.data[pointerToSlot:pointerToSlot+slotSize], slot)
...
func newSlot(offset uint16, length int, flags slotFlag) []byte {
	// Схема упаковки:
