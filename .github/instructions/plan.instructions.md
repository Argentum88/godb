...
*   **Занятие 3.1: Парсер SQL, Volcano executor и минимальный REPL.**
    *   **Теория:** SQL → AST. Контракт между парсером, планировщиком и executor'ом. Модель итератора "Вулкан" (`Next()`). DDL как executor-узел поверх готового системного каталога. REPL переключается с `InMemoryKV` на SQL поверх executor.
...
*   **Занятие 7.2: Уровни изоляции на блокировках.**
    ...
    *   **Практика:** Интеграция `LockManager` в итераторы для реализации этих уровней изоляции. Расширение `UpdateNode`/`DeleteNode`.
