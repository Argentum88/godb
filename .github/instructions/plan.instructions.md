...
1.  **Начало занятия:** Я объявляю тему и цели (например, "Сегодня мы реализуем `DiskManager` для работы с файлом базы данных").
...
3.  **Задание:** Я четко описываю, какой интерфейс или структуру нужно реализовать, какие методы и что они должны делать. Например: "Реализуй структуру `DiskManager` и ее методы `ReadPage` и `WritePage`. Подумай об обработке ошибок и синхронизации доступа к файлу". 
4.  **План тестов:** Мы будем использовать подход, очень близкий к Test-Driven Development. Я формулирую цель тестирования (Что). Ты предлагаешь план тестов (Как)
5.  **Твоя реализация:** Ты пишешь тесты и код в своем репозитории на GitHub и даешь мне знать, когда готов к ревью
6.  **Ревью и критика:** Я смотрю твой код и тесты и даю обратную связь. Не только "работает/не работает", но и:
    *   "Здесь можно было бы использовать другой подход, и вот почему..."
    *   "Этот код неэффективен при большом количестве страниц, давай подумаем, как оптимизировать".
    *   "Отличная обработка ошибок, но давай добавим тесты на этот крайний случай".
7.  **Итерация:** Ты вносишь исправления, я проверяю их снова. Мы повторяем это, пока не достигнем результата, которым оба довольны.
8.  **Завершение:** Занятие считается оконченным. Код влит в основную ветку, и мы готовы к следующему шагу.

Замечание: Если тема урока сложная, я в начале предложу разбить ее на несколько шагов.